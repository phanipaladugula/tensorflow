...
TEST(NamedShardingTest, Equality) {
  ...
  NamedSharding base(mesh_abcde, /*dim_shardings=*/{ds_ab, ds_dc},
                     /*replicated_axes=*/{axis_b},
                     /*unreduced_axes=*/{axis_c});

  EXPECT_EQ(base,
            NamedSharding(mesh_abcde, {ds_ab, ds_dc}, {axis_b}, {axis_c}));

  // Equal even with different mesh axis names
  Mesh mesh_cadbe({2, 4, 3, 8, 2}, {"c", "a", "d", "b", "e"});
  EXPECT_EQ(base,
            NamedSharding(mesh_cadbe, {ds_ab, ds_dc}, {axis_b}, {axis_c}, {}));

  // Equal even with different metadata.
  OpMetadata metadata;
  metadata.set_op_name("foo");
  EXPECT_EQ(base, NamedSharding(mesh_abcde, {ds_ab, ds_dc}, {axis_b}, {axis_c},
                                {}, {metadata}));

  // Different dim_shardings
  EXPECT_NE(base,
            NamedSharding(mesh_abcde, {ds_ab_open, ds_dc}, {axis_b}, {axis_c}));
  EXPECT_NE(base,
            NamedSharding(mesh_abcde, {ds_dc, ds_ab}, {axis_b}, {axis_c}));
  EXPECT_NE(base, NamedSharding(mesh_abcde, {ds_ab}, {axis_b}, {axis_c}));

  // Different replicated_axes
  EXPECT_NE(base,
            NamedSharding(mesh_abcde, {ds_ab, ds_dc}, {axis_d}, {axis_c}));

  // Different unreduced_axes
  EXPECT_NE(base,
            NamedSharding(mesh_abcde, {ds_ab, ds_dc}, {axis_b}, {axis_a}));

  // Different manual_axes
  EXPECT_NE(base, NamedSharding(mesh_abcde, {ds_ab, ds_dc}, {axis_b}, {axis_c},
                                {axis_e}));
