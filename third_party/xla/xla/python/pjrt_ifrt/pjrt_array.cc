...
absl::StatusOr<std::shared_ptr<PjRtBuffer>> PjRtArray::CopySinglePjRtBuffer(
    size_t index, Device* dst_device, std::optional<MemoryKind> dst_memory_kind,
    ArrayCopySemantics semantics) {
  DCHECK(this);
  if (index >= pjrt_buffers_.size()) {
    return InvalidArgument(
        "Index %d is out of bounds for PjRtArray with %d buffers", index,
        pjrt_buffers_.size());
  }
  TF_ASSIGN_OR_RETURN(
    ...
    switch (semantics) {
      case ArrayCopySemantics::kAlwaysCopy: {
        ...
        TF_ASSIGN_OR_RETURN(std::unique_ptr<PjRtBuffer> copied_buffer,
                            pjrt_buffers_[index]->CopyToMemorySpace(
